...
import numpy as np
from SELeCT.Classifier.feature_selection.fisher_score import \
    fisher_score
...
def map_weights(sources, features, normalizer, fingerprints, state_active_non_active_fingerprints, weight_calc, mi_formula=None, weighted=False):
    ...
    t = [f.flat_total_vec for f in fingerprints]
    fingerprint_value_matrix = np.vstack(t)
    fingerprint_count_matrix = np.vstack([f.flat_count_vec for f in fingerprints])
    non_active_value_matrix = {
        k:np.vstack([v[0].flat_total_vec, *[naf.flat_total_vec for naf in v[1].values()]]) for k,v in state_active_non_active_fingerprints.items()
    }
    non_active_count_matrix = {
        k:np.vstack([v[0].flat_count_vec, *[naf.flat_count_vec for naf in v[1].values()]]) for k,v in state_active_non_active_fingerprints.items()
    }
    ...
    for s in sources:
        ...
        for f in features:
            ...
            weights[s][f] = weight_calc(
                ...
                fp_values=fingerprint_value_matrix[:, flat_total_index],
                fp_counts=fingerprint_count_matrix[:, flat_total_index],
                na_values={k:v[:, flat_total_index] for k, v in non_active_value_matrix.items()},
                na_counts={k:v[:, flat_total_index] for k, v in non_active_count_matrix.items()},
                )
...
def calc_weight(s, f, normalizer, fingerprints, state_active_non_active_fingerprints, mi_formula, weighted, flat_weight_vector, weight_func, fp_values=None, fp_counts=None, na_values=None, na_counts=None):
    ...
    weight = weight_func(s, f, normalizer, fingerprints, state_active_non_active_fingerprints, mi_formula,
                         weighted, flat_weight_vector, lower_bound, scaling_factor, scale_factor, scaled_overall_stdev, scaled_overall_mean, fp_values=fp_values, fp_counts=fp_counts, na_values=na_values, na_counts=na_counts)
    return weight

def calc_weight_fisher_overall(s, f, normalizer, fingerprints, state_active_non_active_fingerprints, mi_formula, weighted, flat_weight_vector, fp_values=None, fp_counts=None, na_values=None, na_counts=None):
    return calc_weight(s, f, normalizer, fingerprints, state_active_non_active_fingerprints, mi_formula, weighted, flat_weight_vector, fisher_overall_weight, fp_values=fp_values, fp_counts=fp_counts, na_values=na_values, na_counts=na_counts)

def calc_weight_fisher(s, f, normalizer, fingerprints, state_active_non_active_fingerprints, mi_formula, weighted, flat_weight_vector, fp_values=None, fp_counts=None, na_values=None, na_counts=None):
    return calc_weight(s, f, normalizer, fingerprints, state_active_non_active_fingerprints, mi_formula, weighted, flat_weight_vector, fisher_weight, fp_values=fp_values, fp_counts=fp_counts, na_values=na_values, na_counts=na_counts)

def calc_weight_MI(s, f, normalizer, fingerprints, state_active_non_active_fingerprints, mi_formula, weighted, flat_weight_vector, fp_values=None, fp_counts=None, na_values=None, na_counts=None):
    return calc_weight(s, f, normalizer, fingerprints, state_active_non_active_fingerprints, mi_formula, weighted, flat_weight_vector, MI_weight, fp_values=fp_values, fp_counts=fp_counts, na_values=na_values, na_counts=na_counts)

def calc_weight_cached_MI(s, f, normalizer, fingerprints, state_active_non_active_fingerprints, mi_formula, weighted, flat_weight_vector, fp_values=None, fp_counts=None, na_values=None, na_counts=None):
    return calc_weight(s, f, normalizer, fingerprints, state_active_non_active_fingerprints, mi_formula, weighted, flat_weight_vector, cached_MI_weight, fp_values=fp_values, fp_counts=fp_counts, na_values=na_values, na_counts=na_counts)

def calc_weight_histogramMI(s, f, normalizer, fingerprints, state_active_non_active_fingerprints, mi_formula, weighted, flat_weight_vector, fp_values=None, fp_counts=None, na_values=None, na_counts=None):
    return calc_weight(s, f, normalizer, fingerprints, state_active_non_active_fingerprints, mi_formula, weighted, flat_weight_vector, histogramMI_weight, fp_values=fp_values, fp_counts=fp_counts, na_values=na_values, na_counts=na_counts)

def calc_weight_histogram_covredMI(s, f, normalizer, fingerprints, state_active_non_active_fingerprints, mi_formula, weighted, flat_weight_vector, fp_values=None, fp_counts=None, na_values=None, na_counts=None):
    return calc_weight(s, f, normalizer, fingerprints, state_active_non_active_fingerprints, mi_formula, weighted, flat_weight_vector, histogram_covredMI_weight, fp_values=fp_values, fp_counts=fp_counts, na_values=na_values, na_counts=na_counts)

def fisher_overall_weight(s, f, normalizer, fingerprints, state_active_non_active_fingerprints, mi_formula, weighted, flat_weight_vector, lower_bound, scaling_factor, scale_factor, scaled_overall_stdev, scaled_overall_mean, fp_values=None, fp_counts=None, na_values=None, na_counts=None):
    ...
    for state_id in state_active_non_active_fingerprints:
        ...
        # state_non_active_fp_values = [
        #     v.fingerprint[s][f] for k, v in state_active_non_active_fingerprints[state_id][1].items()]
        ...
        # non_active_means = np.array([state_active_fp_values.fingerprint_values[s][f], *[
        #                             finger['value'] for finger in state_non_active_fp_values]])
        non_active_means = na_values[state_id]
        ...
        # non_active_counts = np.array([state_active_fp_values.fingerprint[s][f]['seen'],  *[
        #     finger['seen'] for finger in state_non_active_fp_values]])
        non_active_counts = na_counts[state_id]
    ...
    # all_fingerprints = [finger.fingerprint[s][f] for finger in fingerprints]
    # means = np.array([finger.fingerprint_values[s][f]
    #                   for finger in fingerprints])
    # means = np.array([finger['value']
    #                   for finger in all_fingerprints])
    means = fp_values
    scaled_means = (means - lower_bound) / scaling_factor
    # counts = np.array([finger.fingerprint[s][f]['seen']
    #                    for finger in fingerprints if finger.fingerprint[s][f]["seen"] is not None])
    # counts = np.array([finger['seen']
    #                    for finger in all_fingerprints if finger["seen"] is not None])
    counts = fp_counts
...
def fisher_weight(s, f, normalizer, fingerprints, state_active_non_active_fingerprints, mi_formula, weighted, flat_weight_vector, lower_bound, scaling_factor, scale_factor, scaled_overall_stdev, scaled_overall_mean, fp_values=None, fp_counts=None, na_values=None, na_counts=None):
    between_segment_fisher_scores = []
    for state_id in state_active_non_active_fingerprints:
        state_active_fp_values = state_active_non_active_fingerprints[state_id][0]
        state_non_active_fp_values = {
...
def MI_weight(s, f, normalizer, fingerprints, state_active_non_active_fingerprints, mi_formula, weighted, flat_weight_vector, lower_bound, scaling_factor, scale_factor, scaled_overall_stdev, scaled_overall_mean, fp_values=None, fp_counts=None, na_values=None, na_counts=None):
...
def cached_MI_weight(s, f, normalizer, fingerprints, state_active_non_active_fingerprints, mi_formula, weighted, flat_weight_vector, lower_bound, scaling_factor, scale_factor, scaled_overall_stdev, scaled_overall_mean, fp_values=None, fp_counts=None, na_values=None, na_counts=None):
    between_segment_MIMs = []
    for state_id in state_active_non_active_fingerprints:
        state_active_fp_values = state_active_non_active_fingerprints[state_id][0]
        state_non_active_fp_values = {
...
def histogramMI_weight(s, f, normalizer, fingerprints, state_active_non_active_fingerprints, mi_formula, weighted, flat_weight_vector, lower_bound, scaling_factor, scale_factor, scaled_overall_stdev, scaled_overall_mean, fp_values=None, fp_counts=None, na_values=None, na_counts=None):
    between_segment_MIMs = []
...
def histogram_covredMI_weight(s, f, normalizer, fingerprints, state_active_non_active_fingerprints, mi_formula, weighted, flat_weight_vector, lower_bound, scaling_factor, scale_factor, scaled_overall_stdev, scaled_overall_mean, fp_values=None, fp_counts=None, na_values=None, na_counts=None):
    flat_ignore_index = normalizer.ignore_indexes[s][f]
    between_segment_MIMs = []
    merged_histogram, merged_bins, merged_count = None, None, None
    for state_id in state_active_non_active_fingerprints:
