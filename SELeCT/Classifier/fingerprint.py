...
from SELeCT.Classifier.matrixSketch import FrequentDirections


class MinimalFingerprint:
    def __init__(self, vec, id):
        self.flat_ignore_vec = vec
        self.id = id
...
class Fingerprint:
    def __init__(self, stats, normalizer, performance_sources=['labels', 'errors', 'error_distances', 'predictions'], performance_features=['FI'], num_bins=None):
        ...
        self.flat_count_vec = np.empty(self.normalizer.total_num_signals)
        ...
        for source in stats.keys():
            for feature in stats[source].keys():
                ...
                self.flat_count_vec[flat_total_index] = 1
    ...
    def reset_feature(self, value, seen=1, stdev=0):
        return {"value": value, "stdev": stdev, "seen": seen, "M": value, "S": math.sqrt(stdev)}
    ...
    def get_current_sum(self):
        ...
        for s in self.sources:
            for f in self.features:
                curr_sum += self.fingerprint_values[s][f]
        return curr_sum
    ...
    def initiate_evolution_plasticity(self):
        ...
        for source in self.sources:
            for feature in self.features:
                ...
                stdev = self.fingerprint[source][feature]["stdev"]
                self.fingerprint[source][feature] = self.reset_feature(value, seen=0.1, stdev=stdev)
                self.flat_total_vec[flat_total_index] = self.fingerprint_values[source][feature]
                self.flat_count_vec[flat_total_index] = 0.1
                if source in self.normalizer.ignore_indexes and feature in self.normalizer.ignore_indexes[source]:
                    flat_ignore_index = self.normalizer.ignore_indexes[source][feature]
                    self.flat_ignore_vec[flat_ignore_index] = self.fingerprint_values[source][feature]
    ...
    def initiate_clean_evolution_plasticity(self, clean_starting_stats):
        ...
        for source in self.sources:
            for feature in self.features:
                ...
                stdev = self.fingerprint[source][feature]["stdev"]
                self.fingerprint[source][feature] = self.reset_feature(value, stdev=stdev)
                self.fingerprint_values[source][feature] = value
                self.flat_total_vec[flat_total_index] = self.fingerprint_values[source][feature]
                self.flat_count_vec[flat_total_index] = 1
                if source in self.normalizer.ignore_indexes and feature in self.normalizer.ignore_indexes[source]:
    ...
    def merge_performance_data(self, performance_fingerprint):
        ...
        for source in self.sources:
            for feature in self.features:
                ...
                self.flat_total_vec[flat_total_index] = self.fingerprint_values[source][feature]
                self.flat_count_vec[flat_total_index] = self.fingerprint[source][feature]["seen"]
                if source in self.normalizer.ignore_indexes and feature in self.normalizer.ignore_indexes[source]:
                    flat_ignore_index = self.normalizer.ignore_indexes[source][feature]
                    self.flat_ignore_vec[flat_ignore_index] = self.fingerprint_values[source][feature]
...
class FingerprintBinningCache(FingerprintCache):
    ...
    def reset_feature(self, value, seen=1, stdev=0):
        return {"value": value, "stdev": stdev, "seen": seen, "M": value, "S": math.sqrt(stdev), 'Range': [value, value], 'Bins': [(value, value) for i in range(self.num_bins)], 'BinsNP': np.full(self.num_bins + 1, value), 'Histogram': [1, *[0 for i in range(1, self.num_bins)]]}
...
class FingerprintSketchCache(FingerprintCache):
    ...
    def reset_feature(self, value, seen=1, stdev=0):
        return {"value": value, "stdev": stdev, "seen": seen, "M": value, "S": math.sqrt(stdev), 'Range': [value, value]}
