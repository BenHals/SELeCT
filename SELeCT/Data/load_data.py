...
from SELeCT.Data.random_tree_sampling_generator import RandomTreeGeneratorSample
from SELeCT.Data.hyper_plane_sampling_generator import HyperplaneSampleGenerator
...
def create_synthetic_concepts(path, name, seed, difficulty=None):
...
def load_synthetic_concepts(name, seed, raw_data_path = None, difficulty=None):
    ...
    if not file_path.exists():
        ...
        create_synthetic_concepts(file_path, name, seed, difficulty=difficulty)
...
def get_inorder_concept_ranges(concepts, seed, concept_length = 5000, repeats = 1, concept_max = -1, repeat_proportion = None, shuffle=True, dropoff=1.0, nforward=1, noise=0.0):
    idx = 0
    positions = []
    if shuffle:
