...
class PollutionSource:
    def __init__(self, x: float, y: float, strength: float, size:float, spread_factor:float, seed:int, min_max_range: Vector, concept_id: int):
        self.x = x
        self.y = y
...
class PollutionEmission:
    def __init__(self, x: float, y: float, r: float, initial_strength: float, spread_factor: float, min_max_range: Vector):
        self.x = x
        self.y = y
        self.r = r
    ...
    def propagate(self, wind_vec: Vector, ts: float):
        ...
        if any((self.strength <= 10, self.min_max_range[1] < self.x, self.x < self.min_max_range[0], self.min_max_range[1] < self.y, self.y < self.min_max_range[0] )):
            self.alive = False
            if not self.child is None:
                self.child.first_emitted = True
        self.last_time_step = ts
    
    def draw(self, world, grid_square_size):
...
class WindSimGenerator(Stream):
    
    def __init__(self, concept: int=2, produce_image: bool=False, num_sensors: int=8, sensor_pattern: str='circle', sample_random_state: int=None):
        super().__init__()
        ...
        if self.sample_random_state is None:
        ...
        self.grid_square_width = self.window_width / self.grid_n
    ...
    def set_concept(self, concept_id: int):
        ...
        strength = self.base_emitter_strength + self.variable_emitter_strength * (get_circle_proportion(self.wind_direction_radians))
        size = self.base_emitter_size
        spread_factor = 1
        self.sources.append(PollutionSource(x, y, strength, size, spread_factor, self.sample_random_state, [0, self.window_width], self.concept))
        spread_factor = self.base_emitter_spread_factor + self.variable_emitter_spread_factor * (get_circle_proportion(self.wind_direction_radians))
        ...
        size = self.base_emitter_size
        spread_factor += 0.02
        self.sources.append(PollutionSource(x2, y2, strength, size, spread_factor, self.sample_random_state + 1, [0, self.window_width], self.concept))
        ...
        size = self.base_emitter_size
        # spread_factor += 0.02
        self.sources.append(PollutionSource(x3, y3, strength, size, spread_factor, self.sample_random_state + 2, [0, self.window_width], self.concept))

    def get_direction_from_concept(self, concept_id: int):
        return 90 * concept_id

    def set_wind(self, concept_id: int=0, direc: float=None, strength: float=None):
        self.concept = concept_id
    ...
    def update_simulation(self):
        ...
        # if self.ex % (1 + (4 * self.concept)) == 0:
        #     for i, s in enumerate(self.sources):
        #         emission = s.emit()
        #         assert emit_test[i] == True
        #         self.pollution.add(emission)
        #         if emission.first_emitted:
        #             self.pollution_chain.append(emission)
        # else:
        #     for i, s in enumerate(self.sources):
        #         assert emit_test[i] == False
    ...
    def collect_sensor_readings(self, world):
        sensor_windows = []
    ...
    def set_sensor_locs(self, world):
        for x, y in self.optimal_sensor_locs:
            for dx in range(-1, 2):
                for dy in range(-1, 2):
    ...
    def update(self, ts, collect_readings = True, draw_intermediary=False):
    ...
    def add_emissions(self, readings):
        X, y = readings
    ...
    def probe_sample(self, draw_intermediary=False):
