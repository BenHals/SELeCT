...
def test_full_comparisons(condaenv=None, runall=False, cpu=1, clean_up=True):
...
def regenerate_test(args, cmd_str, testoutput_p):
    if args.condaenv:
    ...
    with cmd_fn.open('w+') as f:
        f.write(cmd_str)
    pathlib.Path('./comparisons').mkdir(parents=True, exist_ok=True)
    shutil.move(src=str(test_option_fn), dst=str(pathlib.Path('./comparisons') / str(test_option_fn.stem + '.txt')))
    shutil.move(src=str(cmd_fn), dst=str(pathlib.Path('./comparisons') / str(cmd_fn.stem + '.txt')))
    shutil.move(src=str(rslt_fn), dst=str(pathlib.Path('./comparisons')/ str(rslt_fn.stem + '.txt')))
    shutil.move(src=str(data_fn), dst=str(pathlib.Path('./comparisons')/ str(data_fn.stem + '.csv')))
...
if __name__ == "__main__":
    ...
    my_parser.add_argument('--condaenv', default=None, type=str)
    ...
    my_parser.add_argument('--retain', action='store_true')
    ...
    if args.createtests:
        ...
        for cmd_str in cmd_strs:
            regenerate_test(args, cmd_str, testoutput_p)

    # Otherwise, test against the option sets in ./comparisons       
    else:
        test_full_comparisons(args.condaenv, args.runall, args.cpus, not args.retain)
