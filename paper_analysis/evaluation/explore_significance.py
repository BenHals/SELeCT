...
# var = 'drift_width'
# var_vals = [0.0, 25.0, 50.0, 100.0, 250.0, 500.0, 1000.0, 2500.0]
# var = 'noise'
var = 'TMnoise'
var_vals = [0.0, 0.025, 0.05, 0.075, 0.1, 0.25]
...
for var_val in var_vals:
    ...
    sel = selected[selected[var] == var_val]
    runs = sel.groupby(['data_name', 'seed', 'classifier'])[param].mean()
    # runs.unstack('classifier').rank(axis=1)
    run_ranks_df = runs.unstack('classifier').rank(axis=1, ascending=False)
    run_ranks = run_ranks_df.to_numpy()
    rank_labels = list(run_ranks_df.columns)

    avg_ranks = np.average(run_ranks, axis=0)
    cd = Orange.evaluation.compute_CD(avg_ranks, run_ranks.shape[0])
    Orange.evaluation.graph_ranks(avg_ranks, list(map(classifier_name_map.get, rank_labels)), cd=cd)
    plt.savefig(fr"sig\sig-{experiment_name}-{data_set}-{param}-{var}-{var_val}.pdf")
...
for i, data_name in enumerate(data_sets):
    ...
    sns.lineplot(data=s, x=parameter, y='overall_accuracy', hue='classifier', ax=axis, legend=True, style='classifier', dashes=dashes)
...
plt.savefig(f"{parameter}2.pdf", bbox_inches='tight')
plt.show()
# # %%
# data_name_map = {
#     "AQSex": "AQSex",
